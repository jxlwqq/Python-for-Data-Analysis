...
walk.max()
```
现在来看一个复杂点的统计任务——首次穿越时间，即速记漫步过程中第一次到达某个特定值的时间。假设需要了解随机漫步需要多久才能距离初始0点至少10步远（任一方向均可）。np.abs(walk)>=10可以得到一个布尔型数组，它表示的是距离是否达到或超过10，而这里需要得到的是第一个1-或-10的索引。可以用argmax来解决这个问题，它返回的是该布尔型数组第一个最大值的索引（True就是最大值）：
```
(np.abs(walk)>=10).argmax()
```
注意，这里使用argmanx并不是很搞笑，因为它无论如何都会对数组进行完全扫描。在本例中，只要发现一个True，那就确定它是最大值了。

## 一次模拟多个随机漫步

如果希望多个随机漫步过程（比如5000个），只需对上面的代码做一点点修改可以生产所有的随机漫步过程。只要给numpy.random的函数传入一个二元元组就可以产生一个二维数组，然后就可以一次性计算5000个随机漫步过程（一行一个）的累计和了：
```
nwalks=5000
nsteps=1000
...
```
现在计算所有随机漫步过程的最大值和最小值：
```
walks.max()
walks.min()
```
得到这些数据之后，计算30或-30的最小穿越时间。因为着5000个过程中并不是所有都达到了30，在此之前我们需要用any方法进行检查：
```
hits30=(np.abs(walks)>=30).any(1)
hits30.sum() # 到达30或-30的数量
```
然后利用者个布尔型数组选出哪些穿越了30（绝对值）的随机漫步（行），并调用argmax在轴1上获取穿越时间：
```
